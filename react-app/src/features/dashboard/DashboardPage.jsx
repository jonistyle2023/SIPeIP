...
import { ArrowUp, ArrowDown, DollarSign, CheckCircle, AlertTriangle, TrendingUp, TrendingDown, BookOpen, Flag, Target, Users, BarChart } from 'lucide-react';
import { BarChart as ReBarChart, Bar, XAxis, YAxis, CartesianGrid, Tooltip, Legend, ResponsiveContainer } from 'recharts';

const kpiData = [
    { title: 'Proyectos Activos', value: '247', change: '+12%', changeType: 'increase', icon: TrendingUp, color: 'green' },
    { title: 'Inversión Total', value: '$2.4B', change: '+8% vs trimestre anterior', changeType: 'increase', icon: DollarSign, color: 'green' },
    { title: 'Avance Promedio', value: '73%', change: 'sin cambios', changeType: 'neutral', icon: CheckCircle, color: 'yellow' },
    { title: 'Alertas Activas', value: '18', change: '+3 nuevas alertas', changeType: 'decrease', icon: AlertTriangle, color: 'red' },
];
                    ...
                    <div key={kpi.title} className="bg-white p-6 rounded-lg shadow flex justify-between items-start">
                        <div>
                            ...
                            <p className={`text-xs mt-2 flex items-center ${kpi.changeType === 'increase' ? 'text-green-500' : kpi.changeType === 'decrease' ? 'text-red-500' : 'text-gray-500'}`}>
                                {kpi.changeType === 'increase' ? <ArrowUp size={14} className="mr-1"/> : kpi.changeType === 'decrease' ? <ArrowDown size={14} className="mr-1"/> : null}
                                {kpi.change}
                            </p>
                        </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                        ...
                        {indicatorData.map(indicator => (
                            <div key={indicator.name}>
                                <div className="flex justify-between items-center mb-1">
                                    <p className="text-sm font-medium text-gray-700">{indicator.name} <span className="text-xs text-gray-500">({indicator.projects} proyectos)</span></p>
                                    <p className="text-sm font-bold text-gray-800">{indicator.value}%</p>
                                </div>
                                <div className="w-full bg-gray-200 rounded-full h-2.5">
                                    <div className="h-2.5 rounded-full" style={{ width: `${indicator.value}%`, backgroundColor: indicator.value > 75 ? '#22c55e' : indicator.value > 50 ? '#f59e0b' : '#ef4444' }}></div>
                                </div>
                            </div>
                        ))}
                    </div>
                ...
                <div className="bg-white p-6 rounded-lg shadow">
                    ...
                    <div className="space-y-3">
                        {alertData.map(alert => (
                            <div key={alert.project} className={`p-3 rounded-lg bg-${alert.color}-50 border-l-4 border-${alert.color}-400`}>
            ...
            <div className="bg-white p-6 rounded-lg shadow">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                    {planningInstruments.map(item => (
                        <div key={item.name} className={`flex items-center p-4 rounded-lg bg-${item.color}-50 border border-${item.color}-200`}>
                    ...
                    <div>
                        ...
                        <div className="space-y-2">
                            {alignmentData.map(item => (
                                <div key={item.name} className="flex justify-between items-center text-sm p-2 bg-gray-50 rounded-md">
                                    <span>{item.name}</span>
                        ...
                        <div className="space-y-2">
                            {alignmentData.map(item => (
                                <div key={item.name} className="flex justify-between items-center text-sm p-2 bg-gray-50 rounded-md">
                                    ...
                                    <div className="w-1/3 bg-gray-200 rounded-full h-2.5"><div className="bg-green-500 h-2.5 rounded-full" style={{width: `${item.progress}%`}}></div></div>
                                </div>
                            ))}
                        </div>
                    </div>
