        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex justify-center items-center z-80">
            <div className="bg-white dark:bg-slate-800 rounded-lg shadow-xl w-full max-w-2xl border dark:border-slate-700">
                <div className="p-4 border-b dark:border-slate-700 flex justify-between items-center">
                    <h3 className="text-lg font-semibold dark:text-white">{unit ? 'Editar Unidad' : 'Nueva Unidad'}</h3>
                ...
                <form onSubmit={handleSubmit}>
                    <div className="p-6 space-y-4 max-h-[70vh] overflow-y-auto">
                        {/* Campo Nombre */}
                        <div>
                            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">Nombre de la Unidad</label>
                            <input type="text" name="nombre" value={formData.nombre} onChange={handleChange}
                                   className="w-full p-2 border rounded mt-1 dark:bg-slate-700 dark:border-slate-600 dark:text-white" required/>
                        </div>

                        <div>
                            <label htmlFor="padre" className="block text-sm font-medium text-gray-700 dark:text-gray-300">Unidad Padre
                                (Jerarquía)</label>
                            <select
                                id="padre"
                                name="padre"
                        ...
                        <div>
                            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">MacroSector</label>
                        ...
                        {formData.macrosector && (
                            <div>
                                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">Sectores Vinculados</label>
                                <div
                                    className="mt-2 p-3 border rounded-md max-h-48 overflow-y-auto grid grid-cols-1 sm:grid-cols-2 gap-2 dark:border-slate-600 dark:bg-slate-700/50">
                                    {filteredSectores.length > 0 ? (
                                        filteredSectores.map(sector => (
                                            <label key={sector.id}
                                                   className="flex items-center space-x-3 p-2 rounded hover:bg-gray-50 dark:hover:bg-slate-600 cursor-pointer dark:text-gray-200">
                                                <input
                                                    type="checkbox"
                                        ...
                                        <p className="text-sm text-gray-500 dark:text-gray-400 col-span-full">No hay sectores para este
                                            macrosector.</p>
                                    )}
                                </div>
                            </div>
